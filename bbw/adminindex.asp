...
<td width=50px><a href="netmodify.asp?id=<%=rs("id")%>" target="_self">修改</a></td>
<td width=50px><a href="netdel.asp?id=<%=rs("id")%>" target="_self">删除</a></td>
...
<%if nowpage>1 then%>
<a href="adminindex.asp?page=<%=nowpage-1%>">上一页</a>
<%else%>
上一页
<%end if%>
...
<%next%>
<%if nowpage < tatalpages then%>
<a href="adminindex.asp?page=<%=nowpage+1%>">下一页</a>
<%else%>
下一页
<%end if%> 
<input type='text' size='2' value="页数+Enter" onfocus=" javascript:this.value='';"
onfocusout=" javascript:this.value='页数+Enter';" style='height: 18px; width:75;color:DarkGray ; border:1px solid #FFCC99' onkeydown="javascript: if(event.keyCode==13)location='adminindex.asp?page='+this.value;">

<%if nowpage<>1 then%>
<a href="adminindex.asp?page=<%=1%>"> 首页</a>
<%else%>
首页
<%end if%>
<%if nowpage<>tatalpages then %>
<a href="adminindex.asp?page=<%=tatalpages%>">末页</a>
<%else%>
末页
<%end if%>
