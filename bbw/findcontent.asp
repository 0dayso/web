...
<title>文章数据各类信息搜索查找 免费网址信息发布 帮帮网址信息网</title>
...
na=request("na")
if na<>"" then
exec="select * from aab where  content Like '%"&na&"%'  order by id desc " 
...
<%if nowpage>1 then%>
<a href="findcontent.asp?na=<%=(na)%>&page=<%=nowpage-1%>">上一页</a>
<%else%>
上一页
<%end if%>

<%if nowpage < tatalpages then%>
<a href="findcontent.asp?na=<%=(na)%>&page=<%=nowpage+1%>">下一页</a>
<%else%>
下一页
<%end if%> 
<%if nowpage<>1 then%>
<a href="findcontent.asp?na=<%=(na)%>&page=<%=1%>"> 首页</a>
<%else%>
首页
<%end if%>
<%if nowpage<>tatalpages then %>
<a href="findcontent.asp?na=<%=(na)%>&page=<%=tatalpages%>">末页</a>
<%else%>
末页
<%end if%>
