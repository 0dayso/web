...
<title>进入工作-免费网址信息发布-帮帮网址信息网</title>
...
<td><form name="form1" method="post" action="job.asp?na=工作招聘&c=1" > 
<input type="submit" name="Submit" value="点击进入工作招聘类"> 
 </form> 
</td>
<td><form name="form1" method="post" action="job.asp?na=工作求职&c=1" > 
<input type="submit" name="Submit" value="点击进入工作求职类"> 
 </form> </td>
<td><form name="form1" method="post" action="job.asp?na=工作求职&c=1" > 
<input type="submit" name="Submit" value="点击进入工作职场类"> 
 </form> </td>
 </form> 
...
<%if nowpage>1 then%>
<a href="job.asp?na=<%=na%>&c=1&page=<%=nowpage-1%>">上一页</a>
<%else%>
上一页
<%end if%>
...
<a href="job.asp?na=<%=na%>&c=1&page=<%=nowpage+1%>">下一页</a>
<%else%>
下一页
<%end if%> 
<%if nowpage<>1 then%>
<a href="job.asp?na=<%=na%>&c=1&page=<%=1%>"> 首页</a>
<%else%>
首页
<%end if%>

<input type='text' size='2' value="页数+Enter" onfocus=" javascript:this.value='';"
onfocusout=" javascript:this.value='页数+Enter';" style='height: 18px; width:75;color:DarkGray ; border:1px solid #FFCC99' onkeydown="javascript: if(event.keyCode==13)location='job.asp?na=<%=na%>&c=1&page='+this.value;">
<%if nowpage<>tatalpages then %>
<a href="job.asp?na=<%=na%>&c=1&page=<%=tatalpages%>">末页</a>
<%else%>
末页
<%end if%>
