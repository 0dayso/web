...
<title>进入生活-免费网址信息发布-帮帮网址信息网</title>
...
<td><form name="form1" method="post" action="living.asp?na=生活服装&c=1" > 
<input type="submit" name="Submit" value="生活服装类"> 
 </form> 
</td>
<td><form name="form1" method="post" action="living.asp?na=生活饮食&c=1" > 
<input type="submit" name="Submit" value="生活饮食类"> 
 </form> </td>
<td><form name="form1" method="post" action="living.asp?na=生活房子&c=1" > 
<input type="submit" name="Submit" value="生活房子类"> 
 </form> </td>
<td><form name="form1" method="post" action="living.asp?na=生活交通&c=1" > 
<input type="submit" name="Submit" value="生活交通类"> 
 </form> </td>
<td><form name="form1" method="post" action="living.asp?na=生活用品&c=1" > 
<input type="submit" name="Submit" value="生活用品类"> 
 </form> 
</td>
...
<%if nowpage>1 then%>
<a href="living.asp?na=<%=na%>&c=1&page=<%=nowpage-1%>">上一页</a>
<%else%>
上一页
<%end if%>
...
<a href="living.asp?na=<%=na%>&c=1&page=<%=nowpage+1%>">下一页</a>
<%else%>
下一页
<%end if%> 

<input type='text' size='2' value="页数+Enter" onfocus=" javascript:this.value='';"
onfocusout=" javascript:this.value='页数+Enter';" style='height: 18px; width:75;color:DarkGray ; border:1px solid #FFCC99' onkeydown="javascript: if(event.keyCode==13)location='living.asp?na=<%=na%>&c=1&page='+this.value;">

<%if nowpage<>1 then%>
<a href="living.asp?na=<%=na%>&c=1&page=<%=1%>"> 首页</a>
<%else%>
首页
<%end if%>
<%if nowpage<>tatalpages then %>
<a href="living.asp?na=<%=na%>&c=1&page=<%=tatalpages%>">末页</a>
<%else%>
末页
<%end if%>
