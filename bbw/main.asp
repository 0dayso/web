...
&nbsp; &nbsp; &nbsp;<a href="student.asp" target="_parent">进入学习</a><br>
...
<div class="mr" style="font-size:12px;">
<input type="button" value="发布信息" onClick="location.href='sendcontent.asp';">未分类信息数据浏览，以下是全部发布的信息：
...
<%if nowpage>1 then%>
<a href="main.asp?na=<%=na%>&c=1&page=<%=nowpage-1%>">上一页</a>
<%else%>
上一页
<%end if%>
...
<a href="main.asp?na=<%=na%>&c=1&page=<%=nowpage+1%>">下一页</a>
<%else%>
下一页
<%end if%> 

<input type='text' size='2' value="页数+Enter" onfocus=" javascript:this.value='';"
onfocusout=" javascript:this.value='页数+Enter';" style='height: 18px; width:75;color:DarkGray ; border:1px solid #FFCC99' onkeydown="javascript: if(event.keyCode==13)location='main.asp?page='+this.value;">

<%if nowpage<>1 then%>
<a href="main.asp?na=<%=na%>&c=1&page=<%=1%>"> 首页</a>
<%else%>
首页
<%end if%>
<%if nowpage<>tatalpages then %>
<a href="main.asp?na=<%=na%>&c=1&page=<%=tatalpages%>">末页</a>
<%else%>
末页
<%end if%>
共有<%=tatalrecord%>条信息
