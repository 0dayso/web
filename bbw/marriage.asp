...
<title>进入婚恋-免费网址信息发布-帮帮网址信息网</title>
...
<td><form name="form1" method="post" action="marriage.asp?na=婚恋男人&c=1" > 
<input type="submit" name="Submit" value="婚恋男人"> 
 </form> 
</td>
<td><form name="form1" method="post" action="marriage.asp?na=婚恋女人&c=1" > 
<input type="submit" name="Submit" value="婚恋女人"> 
 </form> 
<td><form name="form1" method="post" action="marriage.asp?na=婚恋恋人&c=1" > 
<input type="submit" name="Submit" value="婚恋恋人"> 
 </form> 
<td><form name="form1" method="post" action="marriage.asp?na=婚恋夫妻&c=1" > 
<input type="submit" name="Submit" value="婚恋夫妻"> 
 </form> 
</td>
...
<%if nowpage>1 then%>
<a href="marriage.asp?na=<%=na%>&c=1&page=<%=nowpage-1%>">上一页</a>
<%else%>
上一页
<%end if%>
...
<a href="marriage.asp?na=<%=na%>&c=1&page=<%=nowpage+1%>">下一页</a>
<%else%>
下一页
<%end if%> 
<input type='text' size='2' value="页数+Enter" onfocus=" javascript:this.value='';"
onfocusout=" javascript:this.value='页数+Enter';" style='height: 18px; width:75;color:DarkGray ; border:1px solid #FFCC99' onkeydown="javascript: if(event.keyCode==13)location='marriage.asp?na=<%=na%>&c=1&page='+this.value;">

<%if nowpage<>1 then%>
<a href="marriage.asp?na=<%=na%>&c=1&page=<%=1%>"> 首页</a>
<%else%>
首页
<%end if%>
<%if nowpage<>tatalpages then %>
<a href="marriage.asp?na=<%=na%>&c=1&page=<%=tatalpages%>">末页</a>
<%else%>
末页
<%end if%>
