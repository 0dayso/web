...
<title>买家信息管理 百联百 最大最多最全的商家商品交易市场</title>
...
<table border="0" width="100%" align="center" bgcolor="whitesmoke" color="white"><td><div align="justify"><marquee scrollamount="2" direction="left">
             采购商家最新资讯</div></td></table>
              
<!--#include file="conn.asp"--> 

<% 
exec="select * from cg order by id desc " 
...
<td ><%=rs("cgphone")%></td>
<td ><%=rs("other1")%></td>
<td><a href="opencg.asp?id=<%=rs("id")%>" target="_self">开通使用权限</a></td>
<td ><a href="cgdel.asp?id=<%=rs("id")%>" target="_self">删除</a></td>
</tr>
...
<%if nowpage>1 then%>
<a href="cgadmin.asp?page=<%=nowpage-1%>">上一页</a>
<%else%>
上一页
<%end if%>
...
<%next%>
<%if nowpage < tatalpages then%>
<a href="cgadmin.asp?page=<%=nowpage+1%>">下一页</a>
<%else%>
下一页
<%end if%> 
<input type='text' size='2' value="页数+Enter" onfocus=" javascript:this.value='';"
onfocusout=" javascript:this.value='页数+Enter';" style='height: 18px; width:75;color:DarkGray ; border:1px solid #FFCC99' onkeydown="javascript: if(event.keyCode==13)location='cgadmin.asp?page='+this.value;">

<%if nowpage<>1 then%>
<a href="cgadmin.asp?page=<%=1%>"> 首页</a>
<%else%>
首页
<%end if%>
<%if nowpage<>tatalpages then %>
<a href="cgadmin.asp?page=<%=tatalpages%>">末页</a>
<%else%>
末页
<%end if%>
