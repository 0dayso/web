...
<body>
<div class="bodycss" style=" background-color: white;">
...
<td ><%=rs("id")%></td>
<td ><%=rs("cpname")%></td>
<td ><%=formatnumber(rs("cpprice")*1.1,2,-1)%></td>
<td ><%=formatnumber(rs("cpearn")*1.1,2,-1)%></td>
<td ><%=rs("cpnum")%></td>
<td style="color:red;font-size:16px;"><%=formatnumber(rs("cpprice")*rs("cpnum"),2,-1)%></td>
<td ><a href="viewuptxt.asp?id=<%=rs("id")%>" style="color:yellow;">查看图片</a></td>
<td ><%=rs("how")%></td>
<td ><a href="viewcg.asp?id=<%=rs("id")%>" style="color:yellow;">产品订购</a></td>

<td ><a href="viewcgtxt.asp?id=<%=rs("id")%>" style="color:yellow;">重要信息</a></td>
<td ><%=rs("now")%></td>
<td ><%=rs("xtime")%></td>
</tr>
