...
exec="select * from cp where cpname Like '%"&na&"%' and other3 Like '%"&other33&"%' and way not Like '%"&b&"%'  order by id desc " 
...
<td  style="color:red;font-size:16px;"><%=formatnumber(rs("cpearn")*1.1,2,-1)%></td>
<td ><a href="viewuptxt.asp?id=<%=rs("id")%>" style="color:yellow;">查看图片</a></td>
<td ><%=rs("how")%></td>
<td ><%=rs("now")%></td>
<td ><a href="findcpsave.asp?id=<%=rs("id")%>" style="color:yellow;" >预订此产品</a></td>
